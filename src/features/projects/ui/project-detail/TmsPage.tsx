...
export default function TmsProjectPage() {
  const handleGoBack = () => {
    window.history.back();
  };
          ...
          <div className="flex flex-col md:flex-row md:justify-start md:items-start md:gap-6">
            ...
            <div className="flex-1 w-full md:w-auto">
              <p className="text-slate-50 text-base md:text-lg font-medium font-['Pretendard'] leading-6 md:leading-7">
                기사단말(스마트폰)에서 발생하는GPS 기반 위치 데이터와 운송단계
                이벤트를 실시간 수집 · 전송하여, 전체 운송 흐름을 실시간으로
                모니터링 할 수 있도록 설계된 IoT 모빌리티 관제 플랫폼입니다.
                운송 기사 별 이동 경로, 단계 별 운행상태, 배차 · 작업 · 정산
                흐름을 실시간으로 관제 할 수 있으며, 각 운송 단계에서 발생하는
                이벤트 데이터를 기반으로 실시간 가시화 · 알람 · 업무 처리
                자동화가 가능하도록 구축되었습니다.
              </p>
            </div>
          </div>
